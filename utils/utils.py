...
def gather_features(
    ...
    if gather_with_grad:
        all_image_features = torch.cat(
            distnn.all_gather(image_features), dim=0)
...
def fusion_acc():
    video_labels_list = torch.load(
        'video_sentence_fusion/k400_video_labels.pt')
    video_sim = torch.load('video_sentence_fusion/k400_video_sims.pt')

    sentence_labels_list = torch.load(
        'video_sentence_fusion/k400_sentence_labels.pt')
    ...
    print('a=={}'.format(a), 'b=={}'.format(b), 'top1=={}'.format(
        fusion_prec[0].item()), 'top5=={}'.format(fusion_prec[1].item()))
